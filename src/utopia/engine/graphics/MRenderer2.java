...
//    private int[] pixels; //Vetor com os pixels da tela
    ...
    public MRenderer2(int screenWidth, int screenHeight){
...
//                pixels[x + y * WIDTH] = (int)(System.nanoTime() + x + y); //Preenche com randomness :)
            }
        }
        ...
        camada1.loadImage("res/yeah_bitch3.png");
    ...
    private void drawCanvas(int[] pixels, MCanvas2 canvas){
        ...
        int pixelCanvas = 0; //Indice do pixel dentro do canvas
        
        int yPix = canvas.getPositionY() -1; //-1 para compensar o ++ quando (y=0)
        for (int y=0; y<canvas.getHeight(); y++){
            yPix++;
            
            if ((yPix >= 0) && (yPix < HEIGHT)){ //Se está no Y visivel..
                
                int xPix = canvas.getPositionX() -1; //compensar ++ quando (x=0)
                for (int x=0; x<canvas.getWidth(); x++){
                    xPix++;
                    
                    if ((xPix >= 0) && (xPix < WIDTH)){ //Se está no X visível..
                        int pixelIndex = xPix + (yPix * WIDTH);
                        pixels[pixelIndex] = blend(canvas.getPixelColor(pixelCanvas), pixels[pixelIndex]);                        
                    }
                    
                    pixelCanvas++; //avança um pixel, mesmo que ele não esteja visivel
                }
            }
            else {
                //como não passou pelo FOR, não incrementou naturalmente o indice
                pixelCanvas += canvas.getWidth();
            }
        }
    }
...
/*                    if (pixelMatrix[xPix][yPix] != pixel){
    ...
    private void drugsEffect(int[] pixels){
        for (int y=0; y<HEIGHT; y++){
        	int currentY = y * WIDTH; //calcula o inicio da linha no vetor PIXELS
            for (int x=0; x<WIDTH; x++){
            	if (((pixels[currentY + x] >> 16) & 0xff) < 255) pixels[currentY + x] -= 0x000000; //red
                else pixels[currentY + x] += 0x010000;

                if (((pixels[currentY + x] >> 8) & 0xff) < 255) pixels[currentY + x] -= 0x000100; //green
                else pixels[currentY + x] += 0x010000;

                if ((pixels[currentY + x] & 0xff) < 255) pixels[currentY + x] -= 0x000001; //blue
                else pixels[currentY + x] += 0x010001;        
            }
        }
    }
    ...
    private void preRender(int[] pixels){
        
        //Deve seguir a ordem de empilhamento (fundo primeiro, depois os de cima, topo por ultimo).
        moveThings();
        //drawCanvas(pixels, camada1);        
        drawCanvas(pixels, camada2);
        //drawCanvas(pixels, camada3);
        //if (pixelMatrix[0][0] < 0xff606060) drawCanvas(pixels, camada4);
    }
    
    private void black(int[] pixels){
    ...
    public void render(int[] pixels){
        ...
        preRender(pixels); //Desenha todos os MCanvas na matriz
